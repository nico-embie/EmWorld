...
const Regions = ({ countries, regionName }) => {
  if (!regionName) return <p>error</p>;

  const countriesByRegion = countries
    ? countries.filter((cnt) => {
        return cnt.region.includes(regionName);
      })
    : [];

  const ReturnCountriesByRegion = () => {
    return (
      <ul>
  ...
  return (
    <>
      ...
      <div className="w-5xl mx-auto bg-embie-blue h-full min-h-screen flex flex-col space-y-16">
        ...
        <h2 className="text-2xl text-white flex justify-center">{`${countriesByRegion.length} ${
          countriesByRegion.length > 1 ? "Countries" : " country"
        }`}</h2>
...
export const getStaticPaths = async () => {
  ...
  return {
    paths: countries.map((country) => ({
      params: { name: country.region ? country.region : "Europe" },
    })),
    fallback: true,
  };
};
